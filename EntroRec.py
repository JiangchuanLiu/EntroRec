...
import os
from EntroRec_trainer import EntroRecTrainer
...
def train(
    ...
    category_dict = {"Office_Products":"restaurants", "Industrial_and_Scientific": "industrial and scientific items", "Toys_and_Games": "toys and games"}
    # print(category)

    with open(info_file, 'r') as f:
        info = f.readlines()
        item_name = [_[:-len(_.split('\t')[-1])].strip()  for _ in info]
    ...
    def ndcg_rule_reward(prompts, completions):
        ...
        for i, completion in enumerate(completions):
            # temp1 = completion.strip("\n")[1:-1]
            # temp2 = targets[i].strip("\n")[1:-1]
            # print(f"completion: {temp1}, target: {temp2}")
            if completion.strip("\n")[1:-1] == targets[i].strip("\n")[1:-1]:
                flag = True
                lis.append(0.0)
            else:
    ...
    def cf_reward(prompts, completions):
        ...
        # pred_ids = [item2id[elm.strip("\n").strip("\"")] for elm in completions]
        len_lis = []
        history_ids = []
        ...
        # print(f"seq: {seq.shape}, pred: {pred.shape}")
