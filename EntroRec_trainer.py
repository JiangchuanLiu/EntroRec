...
class EntroRecTrainer(Trainer):
    ...
    def __init__(
        ...
        kl_cov_coef: float = 0.02,  
        select_ratio: float = 0.02,  
        clip_low: float = 0.8,
        clip_high: float = 1.1,
        #* others
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if (
            ...
            save_path = f"wandb_log_step_{self.state.global_step}.csv"
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        kl_cov_penalty = torch.zeros_like(per_token_logps)
        valid_mask = completion_mask.bool()

        if valid_mask.any():

            flat_logps = per_token_logps[valid_mask]


            flat_advs = advantages.unsqueeze(1).expand_as(per_token_logps)[valid_mask]


            covs = (flat_logps - flat_logps.mean()) * (flat_advs - flat_advs.mean())


            k_count = max(1, int(select_ratio * covs.numel()))
            if k_count > 0:
                _, topk_indices = torch.topk(covs, k=k_count, largest=True)


                topk_covs = covs[topk_indices]

                cov_max = topk_covs.max()
                cov_min = topk_covs.min()  
                cov_range = torch.clamp(cov_max - cov_min, min=1e-6)
                if k_count == 1:
                    norm_covs = torch.ones_like(topk_covs)
                else:

                    norm_covs = torch.clamp((topk_covs - cov_min) / cov_range, 0.0, 1.0)

                topk_advs = flat_advs[topk_indices]

                asymmetric_mask = (topk_advs > 0).float()


                flat_kl_values = per_token_kl[valid_mask]


                dynamic_penalty = asymmetric_mask * norm_covs * kl_cov_coef * flat_kl_values[topk_indices]


                sparse_penalty = torch.zeros_like(flat_logps)
                sparse_penalty[topk_indices] = dynamic_penalty


                kl_cov_penalty.masked_scatter_(valid_mask, sparse_penalty)


        per_token_loss_base = torch.exp(per_token_logps - per_token_logps.detach()) * advantages.unsqueeze(1)
        per_token_loss = -(per_token_loss_base - self.beta * per_token_kl - kl_cov_penalty)
        ...
        if self.dapo:
            ...
            s_score = torch.exp((per_token_ratio * completion_mask).sum(dim=1) / completion_mask.sum(dim=1))
            sequence_kl = (per_token_kl * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)
            sequence_kl_cov = (kl_cov_penalty * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)
            loss = -(s_score * advantages - self.beta * sequence_kl - sequence_kl_cov).mean()
        else:
            loss = ((per_token_loss * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
        ...
        if kl_cov_penalty.sum() > 0:
            mean_cov_p = ((kl_cov_penalty * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
            self._metrics["kl_cov_penalty"].append(self.accelerator.gather_for_metrics(mean_cov_p).mean().item())
